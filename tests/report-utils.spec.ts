...
describe('test addReport method', () => {
  ...
  it('should return valid object if right side is null', () => {
    const cov = { covered: 1, pct: 1, skipped: 1, total: 1 }
    const res = addReport(
      { branches: cov, functions: cov, lines: cov, statements: cov },
      null
    )
    expect(res).toBeTruthy()
  })
...
describe('test addCoverageReport method', () => {
  ...
  it('should return valid object if right side is null', () => {
    const res = addCoverageReport(
      { covered: 1, skipped: 1, total: 10, pct: 10 },
      null
    )
    expect(res).toBeTruthy()
  ...
  it('should return valid object if left side is null', () => {
    const res = addCoverageReport(null, {
      covered: 1,
      skipped: 1,
      total: 10,
      pct: 10,
  ...
  it('should add correctly and calculate average percentage', () => {
    const res = addCoverageReport(
      { covered: 5, skipped: 0, total: 10, pct: 50 },
      { covered: 1, skipped: 1, total: 10, pct: 10 }
    )
    expect(res).toBeTruthy()
...
describe('test reportToArray method', () => {
  ...
  it('should return array with correct length and correct items', () => {
    ...
    expect(res[0]).toEqual({
      fileName: 'total',
      report: sampleCoverageSummary.total,
    })
    expect(res[1]).toEqual({
      fileName: 'file1.ts',
      report: sampleCoverageSummary['file1.ts'],
    })
    expect(res[2]).toEqual({
      fileName: 'file2.ts',
      report: sampleCoverageSummary['file2.ts'],
    })
  })
})
